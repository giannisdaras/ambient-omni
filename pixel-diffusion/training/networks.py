...
class AttentionOp(torch.nn.Module):
    ...
    def forward(self, q, k):
        ...
        k_f32 = k.to(torch.float32) / np.sqrt(k.shape[1])
        
        # Compute attention weights
        w = torch.einsum('ncq,nck->nqk', q_f32, k_f32)
    ...
    @staticmethod
    def apply(q, k):
        return AttentionOp.forward(AttentionOp(), q, k)
...
class SongUNet(torch.nn.Module, PyTorchModelHubMixin):
    ...
    def forward_features(self, x, noise_labels, class_labels, augment_labels=None):
        # Mapping.
        emb = self.map_noise(noise_labels)
        emb = emb.reshape(emb.shape[0], 2, -1).flip(1).reshape(*emb.shape) # swap sin/cos
        if self.map_label is not None:
