...
def setup_training_config(preset='edm2-img512-s', **opts):
    ...
    # Dataset.
    c.dataset_kwargs = dnnlib.EasyDict(path=opts.data, 
                                       use_labels=opts.get('cond', True), 
                                       corruption_probability=opts.get('corruption_probability', 0.0),
                                       normalize=False,
    ...
    try:
        ...
        c.dataset_kwargs.max_size = int(len(dataset_obj) * opts.get('dataset_keep_percentage', 1.0))
...
def print_training_config(run_dir, c):
    ...
    dist.print0(f'Dataset size:            {c.dataset_kwargs.get("max_size", "full")}')
    dist.print0(f'Corruption probability: {c.dataset_kwargs.corruption_probability}')
...
@click.option('--snapshot',         help='Interval of network snapshots', metavar='NIMG',       type=parse_nimg, default='4Mi', show_default=True)
@click.option('--checkpoint',       help='Interval of training checkpoints', metavar='NIMG',    type=parse_nimg, default='64Mi', show_default=True)
@click.option('--seed',             help='Random seed', metavar='INT',                          type=int, default=0, show_default=True)
@click.option('-n', '--dry-run',    help='Print training options and exit',                     is_flag=True)
@click.option("--expr_id", help="Experiment ID", type=str, default="edm2-img512-xs")

# Ablations related
@click.option('--dataset_keep_percentage', help='Limit training samples.', type=float, default=1.0, show_default=True)

# Ambient related
@click.option('--annotations_qualities_path', help='Path to the file where the quality annotations are stored.', type=str, default=None)
@click.option('--bad_data_percentage', help='Percentage of data designed "bad" according to global quality', type=float, default=0.8, show_default=True)
@click.option('--bad_data_sigma_min', help='Sigma_min given to "bad" data according to global quality', type=float, default=0.2, show_default=True)
@click.option('--use_ambient_crops', help='Wether to use crops to increase low diffusion time data', metavar='BOOL', type=bool, default=True, show_default=True)
...
def cmdline(outdir, dry_run, **opts):
    ...
    # Add random string to run directory name
    if not dry_run and dist.get_rank() == 0 and not os.path.isdir(outdir):
        random_string = ''.join(random.choices(string.ascii_letters + string.digits, k=5))
        outdir = f"{outdir}-{random_string}"
        os.makedirs(outdir)
