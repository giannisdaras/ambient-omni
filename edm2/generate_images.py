...
def load_hf_checkpoint(repo_id):
    network_kwargs_path = hf_hub_download(repo_id=repo_id, filename="network_kwargs.json")
    network_kwargs = json.load(open(network_kwargs_path, "r", encoding="utf-8"))
    # network_kwargs = dnnlib.EasyDict(class_name="training.networks_edm2.Precond", model_channels=448, dropout=0.1, use_fp16=True)
    inference_kwargs = dnnlib.EasyDict(img_resolution=64, img_channels=4, label_dim=1000)

    net = dnnlib.util.construct_class_by_name(**network_kwargs, **inference_kwargs)
    
    state_dict_path = hf_hub_download(repo_id=repo_id, filename="ema.safetensors")
...
def generate_images(
    ...
    stop_sigma          = 0.0,                  # Stop sampling at this sigma.
    **sampler_kwargs,                           # Additional arguments for the sampler function.
    ...
    if isinstance(net, str):
        ...
        encoder = None
        if net.startswith('giannisdaras/'):
            net = load_hf_checkpoint(net).to(device)
        else:
    ...
    if verbose:
        dist.print0(f'Generating {len(seeds)} images in directory {outdir}...')
                ...
                if len(r.seeds) > 0:
                    ...
                    with torch.no_grad():
                        latents = dnnlib.util.call_func_by_name(func_name=sampler_fn, net=net, noise=r.noise,
                            labels=r.labels, gnet=gnet, randn_like=rnd.randn_like, stop_sigma=stop_sigma, **sampler_kwargs)
...
@click.option('--stop_sigma',               help='Stop sampling at this sigma', metavar='FLOAT',                    type=float, default=0.0, show_default=True)
