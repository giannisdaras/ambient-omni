...
@click.option('--dest',       help='Output directory or archive name', metavar='PATH',  type=str, required=True)
@click.option('--max-images', help='Maximum number of images to output', metavar='INT', type=int)
@click.option('--corruption', help='Corruption', type=str, default='none', show_default=True)
@click.option('--corruption_probability', help='Corruption probability', type=float, default=0.5, show_default=True)
...
def encode(
    ...
    if corruption != 'none':
        corruptions_dict = importlib.import_module(f"noise_configs.{corruption}").corruptions_dict
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        if idx == 1:
            ambient_utils.save_images(img_tensor / 127.5 - 1.0, f"img_{idx}.png"); print(f"Saved image to img_{idx}.png")
        corrupted = False

        if corruption != 'none' and np.random.rand() < corruption_probability:
            corrupted = True
            corruption_name = np.random.choice(list(corruptions_dict.keys()))
            corruption_params = {key: f() for key, f in corruptions_dict[corruption_name].items()}
            corruption_params['sigma'] = 5.0
            corruption_fn = getattr(__import__('ambient_utils.noise'), "apply_" + corruption_name)

            # change range from [0, 255] to [-1, 1]
            img_tensor = img_tensor / 127.5 - 1.0
            img_tensor = corruption_fn(img_tensor, **corruption_params)

            if idx == 1:
                ambient_utils.save_images(img_tensor, f"corrupted_img_{idx}.png"); print(f"Saved image to corrupted_img_{idx}.png")
            img_tensor = ((img_tensor + 1) / 2.) * 255
            img_tensor = img_tensor.clamp(0.0, 255.0).to(torch.uint8)
        
        img_tensor = img_tensor.cuda()
        mean_std = vae.encode_pixels(img_tensor)[0].cpu()

        if idx == 1:
            encoded_latents = vae.encode_latents(mean_std.unsqueeze(0))
            decoded_latents = vae.decode(encoded_latents)
            ambient_utils.save_images(encoded_latents[:, :3], f"latents_{idx}.png"); print(f"Saved image to latents_{idx}.png")
            ambient_utils.save_images(decoded_latents[:, :3] / 127.5 - 1.0, f"decoded_latents_{idx}.png"); print(f"Saved image to decoded_latents_{idx}.png")
        ...
        labels.append([archive_fname, image.label] if image.label is not None else None)
        corruption_labels.append([archive_fname, corrupted])

    metadata = {'labels': labels if all(x is not None for x in labels) else None, 'corruption_labels': corruption_labels if all(x is not None for x in corruption_labels) else None}
    save_bytes(os.path.join(archive_root_dir, 'dataset.json'), json.dumps(metadata))
    close_dest()
