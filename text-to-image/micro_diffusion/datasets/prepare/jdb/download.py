...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('jdb_download.log'),
...
def get_existing_folders(args: argparse.Namespace, split: str) -> set:
    """Get set of folder IDs that already exist in the raw directory."""
    existing_folders = set()
    raw_dir = os.path.join(args.datadir_raw, split, 'imgs')
...
def main():
    ...
    # Skip metadata download if files already exist
    train_anno_path = os.path.join(args.datadir_raw, 'train', 'train_anno_realease_repath.jsonl')
    valid_anno_path = os.path.join(args.datadir_raw, 'valid', 'valid_anno_repath.jsonl')
    
    if not (os.path.exists(train_anno_path) and os.path.exists(valid_anno_path)):
        download_and_process_metadata(args)
    else:
        logger.info("Metadata files already exist, skipping download")

    # Get existing folders for both train and valid splits
    existing_train_folders = get_existing_folders(args, 'train')
    existing_valid_folders = get_existing_folders(args, 'valid')

    # Prepare arguments for multiprocessing, excluding already processed folders
    pool_args = []
    for split in ['train', 'valid']:
        existing_folders = existing_train_folders if split == 'train' else existing_valid_folders
        for i in args.valid_ids:
            if i not in existing_folders:
                pool_args.append((split, i))

    if not pool_args:
        logger.info("All folders have been processed!")
        return

    logger.info(f"Processing {len(pool_args)} remaining folders...")
